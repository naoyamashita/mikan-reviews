...
## 🍊 プロジェクト概要
🍊 みかんの品種ごとに「甘味・酸味・剥きやすさ・コク・じょうのうの薄さ」を5段階で評価し、レビューを蓄積するWebアプリ（PWA対応）。サーバーやDBは持たず、**ブラウザの `localStorage` にデータを保存し、CSVとしてエクスポート/インポートする**シンプルな構成です。
...
```
ID, 日付, 品種, 甘味, 酸味, 剥きやすさ, コク, じょうのうの薄さ, 感想・メモ
```
